...
let desmos_source: string = $state("");

let article_viewport: HTMLElement | null = $state(null);
...
$effect(() => {
	...
	untrack(() => {
		...
		if (subsection[0].kind === "desmos") {
			desmos_source = subsection[0].content;
		}
		else if (current_subsection === 0) {
			desmos_source = "";
		}
